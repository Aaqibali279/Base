 ...
 */
@IBDesignable
open class FloatingTextField: UITextField { // swiftlint:disable:this type_body_length
    /**
    ...
    var btnSecureText = { () -> UIView in
        let rightView = UIView(frame: CGRect(x: 0, y: 0, width: 60, height: 44))
        let btn = UIButton(frame: CGRect(x: 0, y: 8, width: 44, height: 28))
        btn.setImage(Image.slashedEye, for: .normal)
        btn.imageView?.contentMode = .scaleAspectFit
        btn.tintColor = UIColor.primaryColor
        btn.addTarget(self, action: #selector(btnSecureAction), for: .touchUpInside)
        rightView.addSubview(btn)
        
        return rightView
    }()
    
    @objc func btnSecureAction(btn:UIButton){
        isSecureTextEntry = !isSecureTextEntry
        btn.setImage(isSecureTextEntry ? Image.slashedEye : Image.eye, for: .normal)
    }
    ...
    open override func awakeFromNib() {
        ...
        transform = .init(translationX: 16, y: 0)
    ...
    @objc dynamic open var titleFont: UIFont = .systemFont(ofSize: 14, weight: .regular) {
        didSet {
            updateTitleLabel()
        }
    }
    ...
    @IBInspectable dynamic open var disabledColor: UIColor = UIColor.black {
        didSet {
            updateControl()
            updatePlaceholder()
        }
    ...
    @IBInspectable dynamic open var selectedLineColor: UIColor = .blue {
        didSet {
            updateLineView()
        }
    }
...
//        }
    ...
    */
    ...
    fileprivate func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(editing), for: .editingChanged)
    }
    ...
    fileprivate func createErrorLabel() {
        ...
        errorLabel.font = .systemFont(ofSize: 12, weight: .light)
    ...
    */
    ...
    open func errorLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if errorMessagePlacement == .default {
            ...
            if editing {
                let originY = lineRect.origin.y + selectedLineHeight + 0
...
//            return CGRect(x: 0, y: originY, width: bounds.size.width, height: errorHeight())
    ...
    open func lineViewRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        ...
        if errorMessagePlacement == .bottom {
            return CGRect(x: -16, y: textRect(forBounds: bounds).maxY, width: bounds.size.width, height: height)
        } else {
            return CGRect(x: -16, y: bounds.size.height - height, width: bounds.size.width, height: height)
        }
    }
